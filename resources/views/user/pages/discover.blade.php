                        ...
                        <div class="section-banner-search">
                            <form method="GET" action="{{route('search_result')}}">
                                <div class="form-group position-relative mb-0">
                                    <input type="search" name="keyword" class="form-control" placeholder="{{__('main.Search here...')}}">
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    <div class="search-sidebar-wrap">
                        <div class="search-sidebar-box">
                            <h6 class="search-sidebar-box-title">{{__('main.Type')}}</h6>
                            <select class="form-control" id="service-type">
                                <option value="">{{__('main.---SELECT A TYPE---')}}</option>
                                <option value="">{{__('main.Any Type')}}</option>
                                <option value="1">{{__('main.Fixed')}}</option>
                                <option value="2">{{__('main.Bid')}}</option>
                            </select>
                        </div>
                        <div class="search-sidebar-box">
                            <h6 class="search-sidebar-box-title">{{__('main.Price Range')}}</h6>
                        ...
                        <div class="search-sidebar-box">
                            <h6 class="search-sidebar-box-title">{{__('main.Like')}}</h6>
                            <select class="form-control" id="like">
                                <option value="">{{__('main.---SELECT---')}}</option>
                                <option value="DESC">{{__('main.Most liked')}}</option>
                                <option value="ASC">{{__('main.Least liked')}}</option>
                            </select>
                        </div>
                        <div class="search-sidebar-box">
                            <h6 class="search-sidebar-box-title">{{__('main.color')}}</h6>
                            <select class="form-control" id="color">
                                <option VALUE="">{{__('main.---SELECT A COLOR---')}}</option>
                                <option value="">{{__('main.Any Color')}}</option>
                        ...
                        <div class="search-sidebar-box">
                            <h6 class="search-sidebar-box-title">{{__('main.Category')}}</h6>
                            @foreach ($categories as $cat)
                                <label class="checkbox-wrap d-block">
                                    <input type="checkbox" value="{{$cat->id}}" class="category">
                                    <span class="checkmark"></span>
                                    {{$cat->translateOrDefault(Session::get('locale'))->title}}
                                </label>
                            @endforeach

                        </div>
                        <div class="search-sidebar-box">
                            <h6 class="search-sidebar-box-title">{{__('main.Origin')}}</h6>
...
{{--                                            <img src="{{cdnAsset(IMG_SERVICE_PATH,$st->thumbnail)}}" alt="{{__('main.')}}" class="img-fluid">--}}
...
{{--                        </div>--}}
    ...
    <div data-id="{{route('contact_address_list')}}" id="contact_address_list_url"></div>
@endsection
@section('script')
    <script src="{{asset('assets/user/js/pages/discover.js')}}"></script>
    <script>
        getEthAAddress()
        function getEthAAddress() {
            $.ajax({
                url: $('#contact_address_list_url').data('id'),
                type: 'get',
                datatype: 'json',
                beforeSend: function() {
        ...
        async function getNFTs(ethAddress) {
            const options = { chain: '{{env('CHAIN')}}', address: ethAddress };
