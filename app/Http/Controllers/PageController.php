...
class PageController extends Controller
{
    ...
    {
        $categories =   Category::with('services', 'services.author')->get();
    ...
    {
        $eth_address = [];
        $users = User::where('eth_address', '!=', null)->where('eth_address', '!=', '')->get();
        foreach ($users as $user) {
            array_push($eth_address, $user->eth_address);
        }
        return $eth_address;
    }
    ...
    public function nftDetails($token_address, $token_id)
    {
        $data['token_address'] = $token_address;
        $data['token_id'] = $token_id;
        return view('user.pages.nft-details', $data);
    }

    public function ajaxNFTDetails(Request $request)
    {
        $data = ['success' => false, 'value' => null];
        $nft = Service::where('token_address', $request->token_address)->where('token_id', $request->token_id)->with('category')->first();
        if(!is_null($nft)) {
            $data['success'] = true;
            $data['value'] = $nft;
        }
