...
class AuthController extends Controller
{
    ...
    {
        $user = User::where(['username' => $request->get('username',''),'eth_address' => $request->get('eth_address')])->first();
        if(is_null($user)){
             if($this->signUpProcess($request)){
                 $user = User::where('username','=',$request->get('username',''))->first();
                 Auth::loginUsingId($user->id);
                 return response()->json(['status' => true, 'message' => 'reload']);
            }else{
                 return response()->json(['status' => false, 'message' => 'something want wrong']);
            }
        }
        if(!Auth::check()){
            Auth::loginUsingId($user->id);
        }
        return ['status' => true, 'message' => 'reload'];
    }
        ...
        if (file_exists(storage_path('installed'))) {
            ...
            if (Auth::user()) {
                if (Auth::user()->role == USER_ROLE_ADMIN) {
                    ...
                    return redirect()->route('my_collections');
                } else {
                    Auth::logout();
            ...
            $slider = Slider::first();
            $categories_services =   Category::with('services', 'services.author')->paginate(8);
            $latest_max_date = Service::max('created_at');
            $latest_services = Service::orderBy('id', 'DESC')->with('author')->whereDate('created_at', Carbon::parse($latest_max_date)->toDateString())->paginate(8);
            $slide_images = Service::orderBy('id', 'DESC')->with('author')->paginate(8);
    ...
    public function signUpProcess($data)
    {
        try {
            $userData = [
                'first_name' => '',
                    ...
                    if ($user->status == STATUS_SUCCESS) {
                        if (!empty($user->is_verified)) {
                            ...
                            if (Auth::user()->role == USER_ROLE_ADMIN) {
                                ...
                                return response()->json(['success' => true, 'message' => $data['message'], 'data' => ['redirect' => true, 'type' => 'success','returnRoute' => route('my_collections')]]);
                            }
                        } else {
    ...
    public function sendVerifyemail($user, $mail_key)
    {
        ...
        //TODO move to single email send job
    ...
    public function sendForgotMail(Request $request)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                //=====================
                DB::commit();
                return redirect(route('reset_password_page'))->with('success',$data['message']);
            } catch (\Exception $e) {
                DB::rollBack();
                return redirect()->back()->with('dismiss', __('Something went wrong. Please check mail credential.'.$e->getMessage()));
            }
        } else {
