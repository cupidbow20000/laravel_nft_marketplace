...
//        }
        ...
        try {
            ...
            $action = Service::create([
                ...
                'fees_percentage' => 0.2,
                'fees_fixed' => 0,
                'fees_type' => FEES_PERCENTAGE,
                'available_item' => $request->available_item,
                'category_id' => $request->category_id,
                'created_by' => Auth::id(),
                'status' => DRAFT,
                'thumbnail' => $request->image_ipfs,
                'color' => $request->color,
        ...
        if(!empty($action)) {
            addUserNotification('Artwork Added', 'An Artwork is added by you');
    ...
    public function myServiceData(Request $request)
    {
        ...
        return view('user.pages.my-service-data', ['title' => __('main.My Artworks'), 'menu' => 'my-service-data']);
    }
    ...
    public function productToAdmin($id)
    {
        ...
        if(!empty($action)) {
            addUserNotification('Artwork Configure', 'An Artwork has go to admin approval!');
    ...
    public function serviceUpdate(Request $request, $id)
    {
        ...
        if(!empty($action)) {
            addUserNotification('Artwork Update', 'An Artwork is updated by you');
    ...
    public function serviceDelete($id)
    {
        ...
        if(!empty($action)) {
            addUserNotification('Artwork Deleted', 'An Artwork is deleted by you');
    ...
    public function userProductPurchase(Request $request)
    {
        DB::beginTransaction();
        try {
            ...
            Transaction::create([
                'amount' => bcdiv($request->fees,'1000000000000000000',18),
                'from_address' => $request->buyer,
                'to_address' => $request->admin,
                'token_address' => $request->token_address,
                'token_id' => $request->token_id,
            ...
            Transaction::create([
                'amount' => bcdiv($request->amount,'1000000000000000000',18),
                'from_address' => $request->buyer,
                'to_address' => $request->seller,
                'token_address' => $request->token_address,
                'token_id' => $request->token_id,
                'transaction_hash' => randomString(8),
                'fees' => 0,
            ...
            DB::commit();
            return response()->json(['success' => true, 'message' => __('Product Successfully Purchased!')]);
        }
        catch(\Exception $e) {
            DB::rollback();
            return ['status' => false, 'message' => errorHandle($e->getMessage())];
    ...
    public function updateProductBid($request)
    {
        ...
        if(!empty($purchase)) {
            addUserNotification('Purchase', 'Artwork is purchased by you!');
            return response()->json(['success' => true, 'message' => __('Product Successfully Purchased!'), 'data' => ['redirect' => true, 'type' => 'success','returnRoute' => route('bid_history')]]);
        }
        return ['status' => false, 'message' => __('Bid process not done!')];
    }
    ...
    public function resellService(Request $request, $service_id)
    {
        ...
        if(!empty($action)) {
            addUserNotification('Artwork Resell', 'A artwork of you goes to resell!');
