...
class SettingService
{
    ...
    public function saveCommonSetting($request)
    {
        ...
        try {
            ...
            if (isset($request->preloader_logo)) {
                Setting::where('slug', 'preloader_logo')->update(['value' => uploadFile($request->preloader_logo,IMG_PATH,allsetting()['preloader_logo'])]);
            }
            ...
            if (isset($request->meta_title)) {
                Setting::updateOrCreate(['slug' => 'meta_title'], ['value' => $request->meta_title]);
            }
            if (isset($request->meta_description)) {
                Setting::updateOrCreate(['slug' => 'meta_description'], ['value' => $request->meta_description]);
            }
            if (isset($request->cdn_base)) {
                Setting::updateOrCreate(['slug' => 'cdn_base'], ['value' => $request->cdn_base]);
            }
            if (isset($request->is_cdn)) {
                Setting::updateOrCreate(['slug' => 'is_cdn'], ['value' => $request->is_cdn]);
            }
            $response = [
                'success' => true,
